...
#-----------------------------------------------------------------------------------------------------------------------------------------------------#
# Have fun figuring this shit out later mate. No comments for you you fuckin idiot.                                                                   #
#-----------------------------------------------------------------------------------------------------------------------------------------------------#

if (__name__ == "__main__"):
    # objPoints = input("What are the three points? Format as (x1,y1)-(x2,y2)-(x3,y3): ")
    objPoints = "(0, 3 )-(0,0)-(4 ,0)"
    objPoints = objPoints.split("-")
    dataVars =[list(map(float, (re.sub("\(|\)|\ ", "", objPoints[x]).split(",")))) for x in range(len(objPoints))] #Jesus fucking christ what the fuck did i just make?
    locationData = dataVars
    dataVars = pythagManager(dataVars[0], dataVars[1], dataVars[2]) 
    dataVars = [locationData, dataVars, (reverseCos(dataVars[0], dataVars[1], dataVars[2]))]